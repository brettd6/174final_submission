/*
    ...
    poolform: assigns variables to all inputs needed to submit a pool, and sends post request to backend
*/
...
class PoolForm extends React.Component {
    ...
    onValueChange(event) {
        this.setState({hasLifeguard: event.target.value})
    }
    
    //submits pool post to backend
    handleSubmit(event) {
        event.preventDefault();
        if(Number(this.state.timeEnd) <= Number(this.state.timeStart)) alert('Please Enter Valid Availability Times.')
        else {
            Axios.post(process.env.REACT_APP_BACKEND + '/api/uploadpool', {username: this.props.username, size: this.state.size, 
            hasLifeguard: this.state.hasLifeguard, street: this.state.street, city: this.state.city, state: this.state.state,
    ...
    render () {
        return (
        <form className='upload-pool' onSubmit={this.handleSubmit}>
            Does Your Pool Have a LifeGuard?<br></br>
            <label><input type="radio" value ='Yes' checked={this.state.hasLifeguard === 'Yes'} onChange={this.onValueChange}/>Yes</label>
            <label><input type="radio" value ='No' checked={this.state.hasLifeguard === 'No'} onChange={this.onValueChange}/>No</label>
            <br></br>How Large is your Pool?<br></br>
            Volume: <input name="size" type="number" value={this.state.size} onChange={this.handleInputChange}/> ft<sup>3</sup><br></br>
            When is your Pool Available?<br></br>
